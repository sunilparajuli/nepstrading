    ...
    <div class="flex flex-col lg:flex-row gap-8">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="10" 
                            class="w-full border-gray-200 rounded-sm text-sm p-3 bg-gray-50 focus:ring-primary focus:border-primary">{{ old('description') }}</textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Short Description</label>
                        <textarea name="short_description" rows="4" 
                            class="w-full border-gray-200 rounded-sm text-sm p-3 bg-gray-50 focus:ring-primary focus:border-primary">{{ old('short_description') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-sm shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-8 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-gray-50">
                        ...
                        <div id="stock_qty_container" class="{{ old('manage_stock') ? '' : 'hidden' }}">
                    ...
                    <div class="pt-8 border-t border-gray-50">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <input type="datetime-local" name="deal_ends_at" value="{{ old('deal_ends_at') }}" 
                                    class="w-full border-gray-200 rounded-sm text-sm p-3 bg-gray-50">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-400 uppercase mb-2">Deal Total Stock</label>
                                <input type="number" name="deal_total_stock" value="{{ old('deal_total_stock') }}" 
                                    class="w-full border-gray-200 rounded-sm text-sm p-3 bg-gray-50">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="pt-8 border-t border-gray-50">
                        ...
                        <div class="space-y-6">
                            <div class="flex items-center space-x-4">
                                <input type="checkbox" name="allow_add_to_cart" id="allow_add_to_cart" value="1" {{ old('allow_add_to_cart', true) ? 'checked' : '' }}
                                    class="w-4 h-4 text-primary focus:ring-primary border-gray-300 rounded">
                                <label for="allow_add_to_cart" class="text-sm font-medium text-gray-700">Allow customers to add this product to cart?</label>
                            </div>
                            <div id="cart_disabled_message_container" class="{{ old('allow_add_to_cart', true) ? 'hidden' : '' }}">
                                ...
                                <textarea name="cart_disabled_message" rows="3" placeholder="e.g. Please visit our physical shop to purchase this item."
                                    class="w-full border-gray-200 rounded-sm text-sm p-3 bg-gray-50 focus:ring-primary focus:border-primary">{{ old('cart_disabled_message') }}</textarea>
        ...
        <div class="w-full lg:w-1/4 space-y-8">
            <div class="bg-white p-6 rounded-sm shadow-sm border border-gray-100">
                <h3 class="font-bold mb-4 uppercase text-xs tracking-wider text-gray-400 border-b border-gray-50 pb-2">Publish</h3>
                <button type="submit" class="w-full bg-blue-600 text-white rounded-sm py-3 text-sm font-bold hover:bg-blue-700 transition-colors shadow-md">Publish Product</button>
            </div>
            ...
            <div class="bg-white p-6 rounded-sm shadow-sm border border-gray-100">
                ...
                <a href="{{ route('admin.categories.index') }}" class="text-xs text-blue-600 hover:underline">+ Add new category</a>
            </div>
